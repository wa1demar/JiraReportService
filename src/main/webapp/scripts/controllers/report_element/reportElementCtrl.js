(function() {
    ...
    angular
        ...
        .controller('ReportElementCtrl', ReportElementCtrl);

    ReportElementCtrl.$inject = ['$scope', '$routeParams', '$timeout', '$location', 'ReportsFactory', 'ReportFactory', 'ReportWithSprintsAndTeamsFactory'];

    function ReportElementCtrl($scope, $routeParams, $timeout, $location, ReportsFactory, ReportFactory, ReportWithSprintsAndTeamsFactory) {
        var self = this;
        ...
        $scope.updateProgressBar = function (item) {
            ...
            if ($scope.showSprintId === null || $scope.showSprintId === undefined) {
                ...
                for (var index = 0; index < $scope.reportData.sprints.length; index++) {
                    ...
                    //check notCountTarget
                    if (!sprintGlobal.notCountTarget) {
